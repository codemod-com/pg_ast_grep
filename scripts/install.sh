...
# Clone the project
git clone https://github.com/codemod-com/pg_ast_grep "${TEMP_DIR}"
...
# Fetch pg_ast_grep version for pgrx
...
if [[ "$PGRX_VERSION_INSTALLED" != "$PGRX_VERSION" ]]; then
  echo "Installed version of pgrx $PGRX_VERSION_INSTALLED does not match pg_ast_grep version $PGRX_VERSION."
